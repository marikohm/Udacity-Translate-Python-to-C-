...
float sum(vector <vector <float> > grid) {
  float total = 0.0f;
      ...
      for (int column = 0; column < width; column++){
        for ( int dx = -1; dx < 2; dx++){
          for( int dy = -1; dy < 2 ; dy ++){
            new_i = (height + row + dy) % height ; 
            new_j = (width + column + dx) % width ;
            newGrid[new_i][new_j] += window [dx+ 1] [dy + 1] * grid[row] [ column];
          }
        }
      }
    }
